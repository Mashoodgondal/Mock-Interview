...
const NewInterview = () => {
    ...
    const SubmitHandler = async (e) => {
        ...
        // Set default question count if environment variable is not set
        const questionCount = process.env.NEXT_PUBLIC_INTERVIEW_QUESTION_COUNT || 5;

        const InputPrompt = `
Generate exactly ${questionCount} interview questions with detailed answers for the following job details:
...
[
  {
    ...
    "answer": "React is a JavaScript library for building user interfaces. With ${jobExperience} years of experience, I have worked on multiple projects using React for component-based development, state management, and creating responsive web applications."
  }
]

Make sure to:
1. Generate relevant questions based on the job position and tech stack
2. Provide comprehensive answers suitable for someone with ${jobExperience} years of experience
3. Return ONLY the JSON array without any markdown formatting or additional text
4. Each question should be specific to the role and technologies mentioned
`;
        ...
        try {
            ...
            try {
                ...
                // Check if each item has question and answer properties
                parsedJson.forEach((item, index) => {
                    if (!item.question || !item.answer) {
                        throw new Error(`Item at index ${index} missing question or answer`);
                    }
                });

                setjsonResp(cleanedResponse);
            ...
            // Generate unique ID for the mock interview
            const newMockid = uuidv4();
            console.log("Generated Mock ID:", newMockid);

            // Save to database
            try {
                const dbResult = await db.insert(MockInterview).values({
                    mockId: newMockid,
                ...
                alert("Failed to save interview data. Please try again.");
            }
